import {Tab, Segment, Table, Button, Icon} from 'semantic-ui-react';
...
const HistoryDataTable = ({deviceData, objectId}) =>{
  ...
  const removeTableRow = event => {
    ...
    apiAgents.History.delete(deviceId, telemetryid, {'user':objectId}).then(response => {
  ...
  return (
    <Segment color='purple'>
      <Tab.Pane attached={false} >
        <Table className='historyTable' striped size='small'>
          {historyTableHeader()}
          ...
          <Table.Footer>
            <Table.Row>
              ...
              {
                  deviceData.historyData.length > 0?
                  (
